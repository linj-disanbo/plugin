...
function zkspot_sell_nft() {
  echo "=========== # zkspot sell nft order ============="
    local symbol=$1
    local amount=$2
    local privkey=$3
    local accountid=$4
    ...
    local rawData=$(${CLI} zkspot sell_nft2  -a ${amount} -l ${symbol} -r ${rightAsset} -p ${price} --accountId ${accountid} --ethAddress ${ethAddr})
    echo "${rawData}"
    ...
    echo "${hash}"
    query_tx "${CLI}" "${hash}" # bug ErrExecPanic
    local orderID=$(${CLI} tx query -s "${hash}" | jq  ".receipt.logs[1].log.order.orderID")
    echo "orderID: ${orderID}"
}

echo "evmxgo mint nft, transfer to zkspot, then create sell order"
## acc2 maker: 2_1 buy  
## acc3 taker: 2_1 sell
amount=1
nft1=0x66666:11
...
nft3=0x66666:33
## 11
mint_nft ${nft1} ${amount} ${acc2privkey}
mint_nft ${nft1} 10 ${acc2privkey}
## 1
mint_nft ${nft2} ${amount} ${acc2privkey}

## 0
mint_nft ${nft3} ${amount} ${acc2privkey}
burn_nft ${nft3} ${amount} ${acc2privkey}

##
rightAsset=2
price=1111111
evmxgo_transferTo_zkspot ${nft1Symbol} ${amount} ${acc2privkey}
zkspot_sell_nft ${nft1Symbol} ${amount} ${acc2privkey} ${acc2id}  ${acc2eth} ${rightAsset} ${price}
