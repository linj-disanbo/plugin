...
func (a *Action) Deposit(payload *zt.ZkDeposit) (*types.Receipt, error, uint64, *zkHandler) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "checkParam"), 0, nil
	}

	if !et.CheckIsNormalToken(payload.TokenId) {
		return nil, errors.Wrapf(types.ErrNotAllow, "tokenId=%d should less than system NFT base ID=%d", payload.TokenId, zt.SystemNFTTokenId), 0, nil
	}
	...
	if !isSuperManager(cfg, a.fromaddr) && !isVerifier(a.statedb, a.fromaddr) {
		return nil, errors.Wrapf(types.ErrNotAllow, "from addr is not manager"), 0, nil
	}

	//TODO set chainID
	lastPriority, err := getLastEthPriorityQueueID(a.statedb, 0)
	if err != nil {
		return nil, errors.Wrapf(err, "get eth last priority queue id"), 0, nil
	}
	lastPriorityId, ok := big.NewInt(0).SetString(lastPriority.GetID(), 10)
	if !ok {
		return nil, errors.Wrapf(types.ErrInvalidParam, fmt.Sprintf("getID =%s", lastPriority.GetID())), 0, nil
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "db.generateTreeUpdateInfo"), 0, nil
	}

	leaf, err := GetLeafByChain33AndEthAddress(a.statedb, payload.GetChain33Addr(), payload.GetEthAddress(), info)
	if err != nil {
		return nil, errors.Wrapf(err, "db.GetLeafByChain33AndEthAddress"), 0, nil
	}

	tree, err := getAccountTree(a.statedb, info)
	if err != nil {
		return nil, errors.Wrapf(err, "db.getAccountTree"), 0, nil
	}
	...
	if leaf == nil {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "calProof"), 0, nil
		}
		...
		if err != nil {
			return nil, errors.Wrapf(err, "db.AddNewLeaf"), 0, nil
		}
		receipt, err = calProof(a.statedb, info, operationInfo.AccountID, payload.TokenId)
		if err != nil {
			return nil, errors.Wrapf(err, "calProof"), 0, nil
		...
		if err != nil {
			return nil, errors.Wrapf(err, "db.UpdateLeaf"), 0, nil
		}
		receipt, err = calProof(a.statedb, info, leaf.AccountId, payload.TokenId)
		if err != nil {
			return nil, errors.Wrapf(err, "calProof"), 0, nil
	...
	return mergeReceipt(receipts, r), nil, operationInfo.AccountID, newZkHandler(info)
}
...
func (a *Action) Withdraw(payload *zt.ZkWithdraw) (*types.Receipt, error) {
	...
	after := getBranchByReceipt(receipt, operationInfo, leaf.EthAddress, leaf.L2Addr, leaf.PubKey, leaf.ProxyPubKeys, operationInfo.AccountID, operationInfo.TokenID, receipt.Token.Balance)
...
func (a *Action) ContractToTree(payload *zt.ZkContractToTree) (*types.Receipt, error) {
	...
	before := getBranchByReceipt(receipt, operationInfo, leaf.EthAddress, leaf.L2Addr, leaf.PubKey, leaf.ProxyPubKeys, operationInfo.AccountID, operationInfo.TokenID, balance)
...
func (a *Action) Transfer(payload *zt.ZkTransfer) (*types.Receipt, error) {
	...
	before := getBranchByReceipt(receipt, operationInfo, fromLeaf.EthAddress, fromLeaf.L2Addr, fromLeaf.PubKey, fromLeaf.ProxyPubKeys, payload.FromAccountId, payload.TokenId, receipt.Token.Balance)
	//更新fromLeaf
	...
	after := getBranchByReceipt(receipt, operationInfo, fromLeaf.EthAddress, fromLeaf.L2Addr, fromLeaf.PubKey, fromLeaf.ProxyPubKeys, payload.FromAccountId, payload.TokenId, receipt.Token.Balance)
	...
	before = getBranchByReceipt(receipt, operationInfo, toLeaf.EthAddress, toLeaf.L2Addr, toLeaf.PubKey, toLeaf.ProxyPubKeys, payload.ToAccountId, payload.TokenId, balance)
	...
	after = getBranchByReceipt(receipt, operationInfo, toLeaf.EthAddress, toLeaf.L2Addr, toLeaf.PubKey, toLeaf.ProxyPubKeys, payload.ToAccountId, payload.TokenId, receipt.Token.Balance)
	rootHash := zt.Str2Byte(receipt.TreeProof.RootHash)
	kv := &types.KeyValue{
		Key:   getHeightKey(a.height),
		Value: rootHash,
...
func (a *Action) SetPubKey(payload *zt.ZkSetPubKey) (*types.Receipt, error) {
	...
	if payload.PubKeyTy == 0 {
		...
		if zt.Byte2Str(hash.Sum(nil)) != leaf.L2Addr {
...
func (a *Action) SetDefultPubKey(payload *zt.ZkSetPubKey, from string, info *TreeUpdateInfo, leaf *zt.Leaf, operationInfo *zt.OperationInfo) ([]*types.KeyValue, []*types.KeyValue, error) {
	...
	after := getBranchByReceipt(receipt, operationInfo, leaf.EthAddress, leaf.L2Addr, payload.PubKey, nil, operationInfo.AccountID, operationInfo.TokenID, receipt.Token.Balance)
	rootHash := zt.Str2Byte(receipt.TreeProof.RootHash)
	kv := &types.KeyValue{
		Key:   getHeightKey(a.height),
		Value: rootHash,
...
func (a *Action) SetProxyPubKey(payload *zt.ZkSetPubKey, info *TreeUpdateInfo, leaf *zt.Leaf, operationInfo *zt.OperationInfo) ([]*types.KeyValue, []*types.KeyValue, error) {
	...
	after := getBranchByReceipt(receipt, operationInfo, leaf.EthAddress, leaf.L2Addr, leaf.PubKey, receipt.Leaf.GetProxyPubKeys(), operationInfo.AccountID, operationInfo.TokenID, receipt.Token.Balance)
	rootHash := zt.Str2Byte(receipt.TreeProof.RootHash)
	kv := &types.KeyValue{
		Key:   getHeightKey(a.height),
		Value: rootHash,
...
func (a *Action) MakeFeeLog(amount string, info *TreeUpdateInfo, tokenId uint64, sign *zt.ZkSignature) (*types.Receipt, error) {
	...
	before := getBranchByReceipt(receipt, operationInfo, leaf.EthAddress, leaf.L2Addr, leaf.PubKey, leaf.ProxyPubKeys, leaf.GetAccountId(), tokenId, balance)
	...
	after := getBranchByReceipt(receipt, operationInfo, leaf.EthAddress, leaf.L2Addr, leaf.PubKey, leaf.ProxyPubKeys, leaf.GetAccountId(), tokenId, receipt.Token.Balance)
	rootHash := zt.Str2Byte(receipt.TreeProof.RootHash)
	kv := &types.KeyValue{
		Key:   getHeightKey(a.height),
		Value: rootHash,
...
func (a *Action) swapByTransfer(payload *et.ZkTransferWithFee, trade *et.ReceiptSpotTrade, info *TreeUpdateInfo, zklog *zt.ZkReceiptLog) (*types.Receipt, error) {
	...
	before2 := getBranchByReceipt(receipt, operationInfo, toLeaf.EthAddress, toLeaf.L2Addr, toLeaf.PubKey, toLeaf.ProxyPubKeys, payload.ToAccountId, payload.TokenId, balance)
	// 2after
	//更新toLeaf
	...
	after2 := getBranchByReceipt(receipt, operationInfo, toLeaf.EthAddress, toLeaf.L2Addr, toLeaf.PubKey, toLeaf.ProxyPubKeys, payload.ToAccountId, payload.TokenId, receipt.Token.Balance)
	rootHash := zt.Str2Byte(receipt.TreeProof.RootHash)
	kv := &types.KeyValue{
		Key:   getHeightKey(a.height),
		Value: rootHash,
...
func (a *Action) updateLeafRst(info *TreeUpdateInfo, opInfo *zt.OperationInfo, fromLeaf *zt.Leaf,
	...
	before := getBranchByReceipt(receipt, opInfo, fromLeaf.EthAddress, fromLeaf.L2Addr, fromLeaf.PubKey, fromLeaf.ProxyPubKeys, fromLeaf.GetAccountId(), tokenId, receipt.GetToken().GetBalance())
	//更新fromLeaf
	...
	after := getBranchByReceipt(receipt, opInfo, fromLeaf.EthAddress, fromLeaf.L2Addr, fromLeaf.PubKey, fromLeaf.ProxyPubKeys, fromLeaf.GetAccountId(), tokenId, receipt.GetToken().GetBalance())
